...
input_dim = 256
hidden_dim = 256
model = SimpleRNN(len(train_dataset.word2idx_), input_dim, hidden_dim)
optimizer = optim.SGD(model.parameters(), lr=0.5)
criterion = nn.CrossEntropyLoss()
for epoch in range(N_EPOCHS):   	
    train_loss, train_acc, mt_ppl = train(model, train_dataLoader, optimizer, criterion, CLIP, use_cuda = True)
